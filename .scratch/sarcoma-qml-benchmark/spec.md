...
- **Expect near-ceiling classical performance (checked 2026-09-23, real data):** 28 of the 97 LMS patients are uterine-site, 0 of 58 DDLPS — a tissue-of-origin confound for generic features. But the core marker genes (MDM2, CDK4, MYH11, DES, ACTA2) separate the classes just as well, or slightly better, after excluding those 28 (honest 5-fold CV, threshold refit per fold: MDM2 alone reaches 90.7% on the non-uterine-only subset, 93.6% on the full cohort). This is real tumor-lineage biology (12q amplicon vs. smooth-muscle markers), not the uterine confound. **Practical consequence: expect classical models (even a single linear feature) to score very high on Experiment 1 — this pair was chosen for clean separation, so a near-ceiling, possibly saturated result for both quantum and classical models is the likely outcome, not a bug.** Report it as such rather than treating a null quantum-vs-classical gap as a failure.
...
- **Cohort provenance (resolved 2026-09-13):** the spec's 105 LMS / 58 DDLPS traces to a 2016-01-28 GDAC Firehose clinical freeze (`histological_type` field), not to cBioPortal's current `SUBTYPE`/`ICD_O_3_HISTOLOGY`/`CANCER_TYPE_DETAILED` columns — none of which reproduce it standalone. All 163 patients are confirmed to exist in GDC with complete RNA-seq + Genotyping Array (CNA) + WXS (mutation) coverage. The gap: cBioPortal's `sarc_tcga_pan_can_atlas_2018` bundle (the files this pipeline downloads) is missing 5 of the 105 LMS patients (`TCGA-HS-A5N9`, `TCGA-IF-A4AK`, `TCGA-PC-A5DM`, `TCGA-PC-A5DP`, `TCGA-PT-A8TR`) entirely — present and complete in GDC, just not in this specific cBioPortal export. DDLPS's apparent 59-vs-58 mismatch is not a gap: one 2016-labeled DDLPS patient (`TCGA-MO-A47P`) was reclassified to a different liposarcoma subtype since then, and the spec's 58 already reflects that correction. **RESOLVED 2026-09-23**: work with N=155, cBioPortal-only (97 LMS + 58 DDLPS via `ICD_O_3_HISTOLOGY` codes `8890/3`/`8858/3`), zero extra engineering. (Earlier drafts of this line proposed "N=158 (100+58)" — that number was never actually reproducible: `100` comes from `CANCER_TYPE_DETAILED`, `58` from `ICD_O_3_HISTOLOGY`, two different columns' counts glued together. Re-verified directly against the raw file: `ICD_O_3_HISTOLOGY` alone gives 97 LMS, not 100.) The N=163 (105+58, GDC-reprocessed) option is not being pursued for this phase. See `CONTEXT.md §5` and the acquisition log's provenance addendum for detail.
